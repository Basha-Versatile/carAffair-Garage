...
export default function AddExpensePage() {
  ...
  useEffect(() => {
    getExpenseLabels().then(setLabels).catch(() => {});
  ...
  async function handleCreateLabel() {
    ...
    try {
      const label = await createExpenseLabel(newLabelName.trim());
    ...
    <div className="p-5">
      ...
      <div className="glass-card rounded-xl max-w-3xl mx-auto">
        <div className="px-5 sm:px-8 py-6">
          <h2 className="text-xs font-bold tracking-widest text-brand-500 uppercase mb-4">Expense</h2>
      ...
      {/* ── Modals ── */}
      <SelectModal open={labelModalOpen} title="Select Label" items={labels.map((l) => ({ id: l.id, label: l.name }))} selectedId={labelId}
        onSelect={(id) => { const f = labels.find((l) => l.id === id); if (f) { setLabelId(f.id); setLabelName(f.name); } setLabelModalOpen(false); }}
        onClose={() => { setLabelModalOpen(false); setShowNewLabel(false); }}
        actionButton={{ label: "Add New Label", onClick: () => setShowNewLabel(true) }} />
      {showNewLabel && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center animate-fade-in">
          <div className="absolute inset-0 bg-black/30" onClick={() => setShowNewLabel(false)} />
          <div className="relative bg-background rounded-xl shadow-2xl w-full max-w-sm mx-4 p-5 animate-scale-in">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-foreground">New Label</h3>
              <button onClick={() => setShowNewLabel(false)} className="p-1.5 text-muted hover:text-foreground rounded-sm"><X className="w-4 h-4" /></button>
            </div>
            <input type="text" value={newLabelName} onChange={(e) => setNewLabelName(e.target.value)} placeholder="Label name" autoFocus
              className="w-full px-3 py-2.5 bg-dim border border-edge rounded-lg text-sm text-foreground mb-3 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            <button onClick={handleCreateLabel} disabled={!newLabelName.trim()}
              className="w-full py-2.5 bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors">Create Label</button>
          </div>
        </div>
      )}
